...
- **Because `src/proxy.ts` matches `/api/admin/:path*`, Next buffers those request bodies in memory with a 10MB default cap** — anything over it is silently truncated (the request doesn't fail; `request.formData()` then throws mid-parse and 500s). A multi-photo admin upload hit this. Three-part fix: `experimental.proxyClientMaxBodySize: "25mb"` in `next.config.ts` (safety net only — production is still bound by Vercel's 4.5MB function body limit, which config can't raise), the upload form downscales each photo in the browser (2400px long edge, JPEG q90 — lossless in practice since the server re-encodes to 2000px q85) and posts files one per request, and the route returns a 413 instead of an unhandled 500 when `formData()` parsing fails. Files the browser can't decode (e.g. HEIC outside Safari) are sent as-is and can still exceed the production limit — the admin form's help text warns about this
- **"TypeError: ArrayBuffer: SharedArrayBuffer is not allowed." from a blob `put()` in production means sharp is running on its wasm32 fallback.** When sharp's native binary fails to load (on Vercel the traced function bundle missed the linux-x64 binary), it silently falls back to `@img/sharp-wasm32` — a threaded Emscripten build whose whole heap is a SharedArrayBuffer, so `toBuffer()` returns SAB-backed Buffers, which undici's fetch (bundled inside `@vercel/blob`) rejects as a request body. Works locally (native darwin binary loads), breaks only in production. Note the error's stack is useless: undici's fetch wrapper calls `Error.captureStackTrace(err)`, erasing the real call site. Fixed twice over: `processPortfolioImage()` copies SAB-backed output into ordinary memory (and warns, so the fallback is visible in logs), and `outputFileTracingIncludes` pins the `@img/sharp-linux-x64` + `@img/sharp-libvips-linux-x64` packages into the upload route's bundle
