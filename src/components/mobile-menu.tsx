  ...
  return (
    <div className="md:hidden">
      ...
      {/* Portaled to <body>: the header's backdrop-filter would otherwise become
          the containing block for this fixed overlay, pinning inset-0 to the
          header box instead of the viewport (menu would render transparent) */}
      {open &&
        createPortal(
          <div
            role="dialog"
            aria-modal="true"
            aria-label="모바일 메뉴"
            className="fixed inset-0 z-50 flex flex-col bg-cream px-5 md:hidden"
          >
